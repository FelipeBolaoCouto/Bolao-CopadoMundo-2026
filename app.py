...
with aba1:
    ...
    if nome.strip() != "":
        ...
        for jogo in [j for j in JOGOS_COPA if j["fase"] == fase]:
            ...
            palpites_form[jogo['id']] = {"t1": jogo['time1'], "t2": jogo['time2'], "p1": p1, "p2": p2}
            st.write("---")
            
        if st.button("🚀 Salvar Meus Palpites"):
            df_atual = carregar_csv(ARQUIVO_JOGOS, ["Data Hora", "Nome", "Jogo ID", "Confronto", "Placar 1", "Placar 2", "Campeao Apostado", "Artilheiro Apostado"])
            novas_linhas = []
            ...
            st.success("🎉 Seus palpites foram registrados com sucesso!")
    else:
...
with aba3:
    ...
    if senha == "1234":
        ...
        if st.button("💾 Publicar Resultado do Jogo"):
            df_res_atual = carregar_csv(ARQUIVO_RESULTADOS, ["Jogo ID", "Placar 1 Real", "Placar 2 Real"])
        ...
        st.info("Deixe em branco até o final do campeonato.")
        ...
        if st.button("🏆 Publicar Encerramento da Copa"):
            ...
            st.success("🎉 Ganhadores oficiais da Copa publicados!")